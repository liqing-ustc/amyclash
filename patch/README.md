...
- `extra-rules.yaml` — extra rule providers (`extra-rule-providers`) and the rules that reference them (`extra-provider-rules`). A provider's optional `default` key sets which option its generated selector lists first (e.g. `default: DIRECT`, or a region group like `default: 🇺🇸 Auto 美国`); it defaults to `PROXY`.
