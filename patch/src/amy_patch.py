...
def _provider_proxies(default: str | None, regions: list[str]) -> list[str]:
    """Build a provider selector's option list, moving `default` to the front.

    `default` may be any selectable target ("PROXY", "DIRECT", or a region
    group like "🇺🇸 Auto 美国"); it defaults to "PROXY". Unknown values
    (e.g. a region absent from this subscription) fall back to PROXY-first."""
    proxies = ["PROXY", "DIRECT", *regions]
    head = default or "PROXY"
    if head in proxies:
        proxies = [head, *(p for p in proxies if p != head)]
    return proxies
        ...
        proxy_groups.append(
            {
                "name": rule_name,
                "type": "select",
                "proxies": _provider_proxies(providers[provider].get("default"), regions),
            }
        )
