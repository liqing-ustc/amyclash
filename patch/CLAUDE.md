...
- A provider's optional `default` key sets which option its generated selector lists first (e.g. `default: DIRECT`, or a region group like `default: 🇺🇸 Auto 美国`); it defaults to `PROXY`, and an unknown value (a region absent from the subscription) falls back to PROXY-first. The `default` key is provider metadata only — it is stripped from `rule-providers` before the config is dumped, so it must never reach the output.
